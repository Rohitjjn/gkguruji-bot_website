...
// Prevent type errors with framer-motion if types are missing
...
const Logo = ({ className, large = false }: { className?: string, large?: boolean }) => {
  ...
  return (
    <div className={`relative ${className} overflow-hidden rounded-full transform-gpu`}>
...
const QuizOverlay = ({ onClose }: { onClose: () => void }) => {
  ...
  // Memoize shuffled questions to prevent reshuffle on renders
  ...
  if (quizState === 'finished') {
    return (
      <div className="fixed inset-0 z-[100] bg-midnight-950 flex flex-col items-center justify-center p-6 text-center animate-fadeIn">
        <div className="max-w-md w-full bg-white/5  border border-gold-500/30 rounded-3xl p-8 shadow-2xl relative overflow-hidden transform-gpu">
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-midnight-950 flex flex-col">
      ...
      {/* PERFORMANCE FIX: Used scaleX instead of width animation to prevent layout thrashing */}
      <div className="w-full h-1 bg-white/5 overflow-hidden">
        <motion.div 
          className="h-full bg-gold-500 origin-left" 
          initial={{ scaleX: 0 }} 
          animate={{ scaleX: (qIndex + 1) / totalQ }} 
...
const App: React.FC = () => {
  ...
  // PERFORMANCE FIX: Optimized scroll handler to reduce re-renders during resize/zoom
  useEffect(() => {
    const handleScroll = () => {
  ...
  return (
    <div className="min-h-screen font-sans text-slate-800">
        ...
        {legalModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-midnight-950/80 backdrop-blur-sm">
            <motion.div 
              ...
              className="bg-white rounded-[2.5rem] w-full max-w-2xl overflow-hidden shadow-2xl border border-white/20 transform-gpu"
            >
      ...
      <header className={`fixed top-0 w-full z-50 transition-all duration-500 transform-gpu ${scrolled ? 'bg-white border-b border-slate-200 py-4 shadow-sm' : 'bg-transparent py-6'}`}>
        <div className="container mx-auto px-6 md:px-12 flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center gap-4">
            <a href={TELEGRAM_LINK} className="group relative bg-midnight-950 text-white px-7 py-3 rounded-full font-bold shadow-lg overflow-hidden transform-gpu">
      ...
      <div className="md:hidden fixed bottom-6 left-1/2 transform -translate-x-1/2 z-50 w-[90%] max-w-sm">
        <div className="bg-midnight-950/90 backdrop-blur-md rounded-2xl shadow-2xl p-2 flex justify-between items-center border border-white/10">
        ...
        <div className="container mx-auto px-6 md:px-12 relative z-10">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <motion.div initial={false} animate="visible" variants={staggerContainer}>
              ...
              <motion.div variants={fadeInUp} className="flex flex-col gap-4">
                ...
                <div className="flex flex-col sm:flex-row gap-4 mt-2">
                  <a href={TELEGRAM_LINK} className="bg-gold-500 text-midnight-950 px-8 py-4 rounded-xl font-bold text-lg shadow-glow-gold flex items-center justify-center gap-3 transition-transform hover:-translate-y-1 transform-gpu">{t.hero.ctaPrimary} <Zap size={20} fill="currentColor" /></a>
            ...
            <div className="relative hidden lg:block perspective-1000">
               <div className="relative w-full max-w-sm mx-auto aspect-[4/5] bg-white/10  border border-white/20 rounded-[2.5rem] shadow-2xl p-8 flex flex-col justify-between animate-float transform-gpu">
        ...
        <div className="container mx-auto px-6 md:px-12">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {t.features.list.map((feature, idx) => (
              <motion.div key={feature.id} initial={false} whileInView="visible" viewport={{ once: true, margin: "-50px" }} variants={fadeInUp} className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm hover:shadow-xl transition-all group transform-gpu">
      ...
      <section id="pricing" className="py-24 bg-white">
         ...
         <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto px-6">
            {t.pricing.plans.map((plan, idx) => (
              <div key={idx} className={`relative rounded-[2rem] p-8 transition-all border overflow-hidden transform-gpu ${plan.id === 'basic' ? 'bg-gradient-to-br from-yellow-50 to-yellow-100 border-pink-300 text-slate-800' : plan.id === 'pro' ? 'bg-gradient-to-br from-red-600 to-red-700 text-white shadow-2xl scale-105 border-red-400 z-10' : 'bg-white border-slate-200 text-slate-800'}`}>
                {plan.recommended && (
                  <div className="absolute top-0 right-0 px-5 py-1.5 rounded-bl-2xl text-[10px] font-bold uppercase tracking-widest bg-black text-red-400 shadow-lg">Most Popular</div>
                )}
