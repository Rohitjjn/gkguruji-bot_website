...
import { 
  ...
  Star, Zap, Home, Layers, CreditCard, Sparkles, Trophy,
  Play
} from 'lucide-react';
...
// --- OPTIMIZATION START ---
// Using strict memoization to prevent re-renders
const motion = fMotion as any;

// 1. Static Components (Will never re-render)
const IconMap = memo(({ name, className }: { name: string; className?: string }) => {
  const icons: any = { Brain, BookOpen, TrendingUp, Download, Award, Target };
  const Icon = icons[name] || Star;
  return <Icon className={className} />;
});

const Logo = memo(({ className, large = false }: { className?: string, large?: boolean }) => {
  const [error, setError] = useState(false);
  const [loading, setLoading] = useState(true);

  if (error) {
    return (
      <div className={`relative flex items-center justify-center bg-midnight-950 border border-gold-500/50 rounded-full shadow-lg overflow-hidden ${className}`} style={{ aspectRatio: '1/1' }}>
  ...
  return (
    <div className={`relative ${className} overflow-hidden rounded-full transform-gpu bg-slate-100`} style={{ aspectRatio: '1/1' }}>
      {/* Skeleton loader instead of pulsing div to save CPU */}
      ...
      <img 
        ...
        loading="lazy"
        decoding="async" // Better performance for image decoding
      />
    </div>
  );
});

// 2. Simplified Animations (Reduced complexity)
const fadeInUp = {
  hidden: { opacity: 0, y: 20 }, // Reduced distance
  visible: { opacity: 1, y: 0, transition: { duration: 0.4, ease: "easeOut" } } // Faster duration
};
...
// 3. ISOLATED TIMER COMPONENT (Critical Fix)
// This prevents the entire Quiz Modal from re-rendering every second
const QuizTimer = memo(({ 
  ...
  return (
    <div className={`text-xl font-mono font-bold ${timeLeft <= 3 ? 'text-red-500' : 'text-white'}`}>
...
const QuizOverlay = ({ onClose }: { onClose: () => void }) => {
  ...
  // Memoize questions once
  const shuffledQuestions = useMemo(() => [...QUIZ_DATA].sort(() => Math.random() - 0.5), []);
      ...
      <div className="fixed inset-0 z-[100] bg-midnight-950 flex flex-col items-center justify-center p-6 text-center animate-fadeIn">
        <div className="max-w-md w-full bg-white/5 border border-gold-500/30 rounded-3xl p-8 shadow-2xl relative overflow-hidden">
          ...
          <div className="space-y-3">
             <a href={TELEGRAM_LINK} className="w-full flex items-center justify-center gap-2 bg-gold-500 hover:bg-gold-400 text-midnight-950 py-4 rounded-xl font-bold transition-all">
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-midnight-950 flex flex-col">
      <div className="px-6 py-4 flex items-center justify-between border-b border-white/10 bg-midnight-900">
         ...
         {/* Isolated Timer to prevent re-renders */}
         {quizState === 'question' ? (
           <QuizTimer duration={10} onTimeUp={handleTimeUp} resetTrigger={qIndex} />
         ) : (
           <div className="text-white text-sm font-bold animate-pulse">Next Question...</div>
         )}
      ...
      {/* Simplified Progress Bar (CSS Transition instead of Framer Motion) */}
      <div className="w-full h-1 bg-white/5 overflow-hidden">
        <div 
          className="h-full bg-gold-500 transition-transform duration-500 ease-out origin-left"
          style={{ transform: `scaleX(${(qIndex + 1) / totalQ})` }}
        ></div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 pb-20">
         <div className="max-w-2xl mx-auto mt-4 md:mt-10">
            ...
            {/* Added translate-z-0 to force GPU layer */}
            <h3 className="text-xl md:text-2xl font-bold text-white leading-relaxed mb-8 transform-gpu">{currentQ.question}</h3>
            <div className="space-y-3">
               {currentQ.options.map((opt, idx) => {
                 ...
                 return (
                   <button key={idx} disabled={quizState === 'feedback'} onClick={() => handleOptionClick(opt)} className={`w-full p-4 rounded-xl border text-left font-medium transition-colors duration-200 flex justify-between items-center ${stateClass}`}>
                     <span>{opt}</span>
            ...
            {/* Conditional Rendering without AnimatePresence for better performance */}
            {quizState === 'feedback' && (
               <div className="mt-6 p-4 rounded-xl bg-gold-500/10 border border-gold-500/20 animate-fadeIn">
                  <p className="text-xs font-bold text-gold-400 uppercase tracking-widest mb-2">Explanation</p>
...
const App: React.FC = () => {
  ...
  // REMOVED: All Scroll Listeners. 
  // We use CSS 'scroll-behavior: smooth' in global CSS and simple sticky positioning.

  const scrollToSection = (id: string) => {
    setActiveSection(id);
    const element = document.getElementById(id);
    if (element) {
      // Use native smooth scroll which is optimized by browser
      element.scrollIntoView({ behavior: 'smooth' });
    }
  };

  return (
    <div className="min-h-screen font-sans text-slate-800 bg-slate-50 selection:bg-gold-500/30">
      <AnimatePresence>
        {isQuizActive && <QuizOverlay onClose={() => setIsQuizActive(false)} />}
      </AnimatePresence>
      
      <AnimatePresence>
        {legalModal && (
          // REMOVED: backdrop-blur (Cause of zoom lag). Replaced with solid semi-transparent bg.
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-midnight-950/95">
            <motion.div 
              initial={{ opacity: 0, scale: 0.95 }} 
              animate={{ opacity: 1, scale: 1 }} 
              exit={{ opacity: 0, scale: 0.95 }} 
              transition={{ duration: 0.2 }}
              className="bg-white rounded-[2rem] w-full max-w-2xl overflow-hidden shadow-2xl border border-white/20 transform-gpu"
            >
                ...
                {legalModal === 'privacy' ? (
                  ...
                  <div className="space-y-6">
                    <section>
                      ...
                      <p>By accessing the GK Guruji website or Telegram bot, you agree to be bound by these Terms of Service.</p>
                    </section>
                    <section>
                      <h4 className="text-midnight-950 font-bold text-base mb-2">2. Intellectual Property</h4>
                      <p>All questions, explanations, graphics, and logos are the property of GK Guruji.</p>
                    </section>
                  </div>
                )}
              </div>
      ...
      {/* FIXED HEADER: Removed state-based color change. Always white bg for stability during zoom. */}
      <header className="sticky top-0 w-full z-50 bg-white/95 border-b border-slate-200 py-3 shadow-sm supports-[backdrop-filter]:bg-white/80">
        <div className="container mx-auto px-6 md:px-12 flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center gap-4">
            <a href={TELEGRAM_LINK} className="group relative bg-midnight-950 text-white px-6 py-2.5 rounded-full font-bold shadow-md overflow-hidden transform-gpu hover:shadow-lg transition-all">
      ...
      {/* MOBILE MENU: Removed backdrop-blur */}
      <div className="md:hidden fixed bottom-6 left-1/2 transform -translate-x-1/2 z-50 w-[90%] max-w-sm">
        <div className="bg-midnight-950 rounded-2xl shadow-xl p-2 flex justify-between items-center border border-white/10">
          {[{ id: 'hero', icon: Home, label: 'Home' }, { id: 'features', icon: Layers, label: 'Features' }, { id: 'pricing', icon: CreditCard, label: 'Plans' }].map((item) => (
            <button key={item.id} onClick={() => scrollToSection(item.id)} className={`flex-1 flex flex-col items-center py-2 rounded-xl transition-all ${activeSection === item.id ? 'text-gold-400 bg-white/10' : 'text-slate-400'}`}>
              <IconMap name={item.id === 'hero' ? 'Home' : item.id === 'features' ? 'Layers' : 'CreditCard'} className="w-5 h-5 mb-1" />
              <span className="text-[10px] font-medium mt-1">{item.label}</span>
            </button>
          ))}
          <a href={TELEGRAM_LINK} className="flex-1 flex flex-col items-center py-2 text-gold-500">
            <div className="w-10 h-10 bg-gold-500 rounded-full flex items-center justify-center text-midnight-950 shadow-md -mt-6 border-4 border-midnight-900"><Send size={18} fill="currentColor" /></div>
            <span className="text-[10px] font-bold mt-1">Start</span>
          </a>
        </div>
      </div>

      <section id="hero" className="relative pt-20 pb-20 md:pt-32 md:pb-32 overflow-hidden bg-midnight-950 text-white">
        <div className="container mx-auto px-6 md:px-12 relative z-10">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            {/* Added viewport once=true to stop animations re-triggering on scroll up/down */}
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer}>
              ...
              <motion.div variants={fadeInUp} className="flex flex-col gap-4">
                 <button onClick={() => setIsQuizActive(true)} className="w-full sm:w-auto group relative bg-gradient-to-r from-red-600 to-red-500 text-white px-8 py-5 rounded-2xl font-bold text-lg shadow-xl overflow-hidden transform-gpu">
                ...
                <div className="flex flex-col sm:flex-row gap-4 mt-2">
                  <a href={TELEGRAM_LINK} className="bg-gold-500 text-midnight-950 px-8 py-4 rounded-xl font-bold text-lg shadow-lg flex items-center justify-center gap-3 transition-transform hover:-translate-y-1 transform-gpu">{t.hero.ctaPrimary} <Zap size={20} fill="currentColor" /></a>
            ...
            {/* Simplified 3D Card - Removed float animation to save GPU */}
            <div className="relative hidden lg:block">
               <div className="relative w-full max-w-sm mx-auto aspect-[4/5] bg-white/10 border border-white/20 rounded-[2.5rem] shadow-2xl p-8 flex flex-col justify-between transform-gpu">
        ...
        <div className="container mx-auto px-6 md:px-12">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {t.features.list.map((feature, idx) => (
              <motion.div key={feature.id} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeInUp} className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm hover:shadow-xl transition-all group transform-gpu">
      ...
      <section id="pricing" className="py-24 bg-white">
         ...
         <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto px-6">
            {t.pricing.plans.map((plan, idx) => (
              <div key={idx} className={`relative rounded-[2rem] p-8 transition-all border overflow-hidden transform-gpu ${plan.id === 'basic' ? 'bg-gradient-to-br from-yellow-50 to-yellow-100 border-pink-300 text-slate-800' : plan.id === 'pro' ? 'bg-gradient-to-br from-red-600 to-red-700 text-white shadow-xl scale-105 border-red-400 z-10' : 'bg-white border-slate-200 text-slate-800'}`}>
                {plan.recommended && (
                  <div className="absolute top-0 right-0 px-5 py-1.5 rounded-bl-2xl text-[10px] font-bold uppercase tracking-widest bg-black text-red-400 shadow-md">Most Popular</div>
                )}
                {!plan.recommended && plan.badge && (
                  <div className="absolute top-0 right-0 px-5 py-1.5 rounded-bl-2xl text-[10px] font-bold uppercase tracking-widest bg-gradient-to-r from-pink-500 to-pink-600 text-white shadow-md">{plan.badge}</div>
                )}
