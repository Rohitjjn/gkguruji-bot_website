      ...
      <section id="pricing" className="py-24 bg-white">
         ...
         <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto px-6">
            {t.pricing.plans.map((plan, idx) => (
              <div key={idx} className={`relative rounded-[2rem] p-8 transition-all border overflow-hidden ${plan.recommended ? 'bg-gradient-to-br from-red-600 to-red-700 text-white shadow-2xl scale-105 border-red-400' : plan.id === 'basic' ? 'bg-gradient-to-br from-amber-50 to-orange-50 border-amber-200 text-slate-800' : 'bg-white border-slate-200 text-slate-800'}`}>
                {plan.recommended && (
                  <div className="absolute top-0 right-0 px-5 py-1.5 rounded-bl-2xl text-[10px] font-bold uppercase tracking-widest bg-white text-red-600 shadow-lg">Most Popular</div>
                )}
                {!plan.recommended && plan.badge && (
                  <div className="absolute top-0 right-0 px-5 py-1.5 rounded-bl-2xl text-[10px] font-bold uppercase tracking-widest bg-gradient-to-r from-amber-400 to-orange-500 text-white shadow-lg">{plan.badge}</div>
                )}
