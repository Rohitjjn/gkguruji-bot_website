      ...
      <section id="pricing" className="py-24 bg-white">
         ...
         <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto px-6">
            {t.pricing.plans.map((plan, idx) => (
              <div key={idx} className={`relative rounded-[2rem] p-8 transition-all border overflow-hidden ${plan.id === 'basic' ? 'bg-gradient-to-br from-yellow-50 to-sky-100 border-yellow-300 text-slate-800' : plan.id === 'pro' ? 'bg-gradient-to-br from-red-600 to-red-700 text-white shadow-2xl scale-105 border-black-400' : 'bg-white border-slate-200 text-slate-800'}`}>
                {plan.recommended && (
                  <div className="absolute top-0 right-0 px-5 py-1.5 rounded-bl-2xl text-[10px] font-bold uppercase tracking-widest bg-black text-blue-400 shadow-lg">Most Popular</div>
                )}
