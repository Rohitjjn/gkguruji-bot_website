...
const QuizOverlay = ({ onClose }: { onClose: () => void }) => {
  ...
  if (quizState === 'finished') {
    return (
      <div className="fixed inset-0 z-[100] bg-midnight-950 flex flex-col items-center justify-center p-6 text-center animate-fadeIn">
        <div className="max-w-md w-full bg-white/5 backdrop-blur-xl border border-gold-500/30 rounded-3xl p-8 shadow-2xl relative overflow-hidden">
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-midnight-950 flex flex-col">
      <div className="px-6 py-4 flex items-center justify-between border-b border-white/10 bg-midnight-900/50 backdrop-blur-md">
...
const App: React.FC = () => {
    ...
    const handleScroll = () => {
      ...
      for (const section of sections) {
        ...
        if (el) {
          ...
          if (rect.top >= -100 && rect.top <= 400) setActiveSection(section);
        }
      }
    };
  ...
  return (
    <div className="min-h-screen font-sans text-slate-800">
      ...
      <header className={`fixed top-0 w-full z-50 transition-all duration-500 ${scrolled ? 'bg-white/80 backdrop-blur-md border-b border-slate-200 py-4 shadow-sm' : 'bg-transparent py-6'}`}>
      ...
      <div className="md:hidden fixed bottom-6 left-1/2 transform -translate-x-1/2 z-50 w-[90%] max-w-sm">
        <div className="bg-midnight-950/90 backdrop-blur-xl rounded-2xl shadow-2xl p-2 flex justify-between items-center border border-white/10">
        ...
        <div className="container mx-auto px-6 md:px-12 relative z-10">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            <div className="relative hidden lg:block perspective-1000 animate-float">
               <div className="relative w-full max-w-sm mx-auto aspect-[4/5] bg-white/10 backdrop-blur-xl border border-white/20 rounded-[2.5rem] shadow-2xl p-8 flex flex-col justify-between">
        ...
        <div className="container mx-auto px-6 md:px-12">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {t.features.list.map((feature, idx) => (
              <motion.div key={feature.id} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeInUp} className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm hover:shadow-xl transition-all group">
      ...
      <section id="pricing" className="py-24 bg-white">
         ...
         <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto px-6">
            {t.pricing.plans.map((plan, idx) => (
              <div key={idx} className={`relative rounded-[2rem] p-8 transition-all border ${plan.recommended ? 'bg-midnight-950 text-white shadow-2xl scale-105 border-gold-500/20' : 'bg-white border-slate-200 text-slate-800'}`}>
                {plan.badge && <div className="absolute top-0 right-0 px-5 py-1.5 rounded-bl-2xl text-[10px] font-bold uppercase tracking-widest bg-gold-500 text-midnight-950">{plan.badge}</div>}
                <h3 className={`text-lg font-bold mb-2 ${plan.recommended ? 'text-gold-400' : 'text-slate-400'}`}>{plan.name}</h3>
                <div className="flex items-baseline gap-1 mb-8"><span className="text-5xl font-serif font-bold">{plan.price}</span><span className="text-sm opacity-60">{plan.priceSub}</span></div>
                <ul className="space-y-4 mb-8">{plan.features.map((f, i) => (<li key={i} className="flex items-start gap-3 text-sm"><Check size={16} className="text-gold-400 mt-0.5" />{f}</li>))}</ul>
                <a href={TELEGRAM_LINK} className={`w-full block py-4 rounded-xl font-bold text-center transition-colors ${plan.recommended ? 'bg-gold-500 text-midnight-950 shadow-glow-gold' : 'bg-slate-100 text-midnight-950 hover:bg-slate-200'}`}>{plan.buttonText}</a>
              </div>
            ))}
         </div>
      </section>
