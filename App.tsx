...
const IconMap = ({ name, className }: IconMapProps) => {
  const icons: Record<string, React.ComponentType<any>> = { 
    Brain, BookOpen, TrendingUp, Download, Award, Target 
  };
...
const Logo = ({ className, large = false }: LogoProps) => {
  ...
  if (error) {
    return (
      <div 
        className={`relative flex items-center justify-center bg-midnight-950 border border-gold-500/50 rounded-full shadow-glow-gold overflow-hidden ${className}`} 
        style={{ aspectRatio: '1/1' }}
      >
        <div className="absolute inset-0 bg-gold-400/10"></div>
        <span className={`${large ? "text-3xl" : "text-base"} font-serif font-bold text-gold-400 z-10 tracking-tighter`}>
          GK
        </span>
      </div>
    );
  ...
  return (
    <div className={`relative ${className} overflow-hidden rounded-full`}>
      ...
      <img 
        src={LOGO_URL} 
        alt="GK Guruji" 
        className={`${className} object-contain transition-opacity duration-300 ${loading ? 'opacity-0' : 'opacity-100'}`}
...
const QuizOverlay = ({ onClose }: QuizOverlayProps) => {
      ...
      <div className="fixed inset-0 z-[100] bg-midnight-950 flex flex-col items-center justify-center p-6 text-center animate-fadeIn">
        <div className="max-w-md w-full bg-white/5 border border-gold-500/30 rounded-3xl p-8 shadow-2xl relative overflow-hidden">
          ...
          <div className="bg-midnight-900 rounded-2xl p-6 mb-8 border border-white/10">
            ...
            <p className="text-5xl font-bold text-transparent bg-clip-text bg-gold-gradient">
              {score}/{totalQ}
            </p>
          </div>
          <div className="space-y-3">
            <a 
              href={TELEGRAM_LINK} 
              className="w-full flex items-center justify-center gap-2 bg-gold-500 hover:bg-gold-400 text-midnight-950 py-4 rounded-xl font-bold transition-all shadow-glow-gold"
            >
              <Send size={18} /> Join Bot
            </a>
            <button 
              onClick={onClose} 
              className="w-full py-4 rounded-xl font-bold text-slate-400 hover:text-white hover:bg-white/5 transition-all"
            >
              Back to Home
            </button>
          </div>
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-midnight-950 flex flex-col">
      <div className="px-6 py-4 flex items-center justify-between border-b border-white/10 bg-midnight-900/50">
        ...
        <button 
          onClick={onClose} 
          className="p-2 bg-white/10 rounded-full text-slate-300 hover:bg-red-500/20 hover:text-red-400 transition"
        >
          <X size={20} />
        </button>
      </div>
            ...
            {currentQ.options.map((opt, idx) => {
              ...
              return (
                <button 
                  ...
                  className={`w-full p-4 rounded-xl border text-left font-medium transition-all duration-200 flex justify-between items-center ${stateClass}`}
                >
                  <span>{opt}</span>
                  {quizState === 'feedback' && opt === currentQ.correctAnswer && (
                    <Check size={18} className="text-green-400" />
                  )}
                  {quizState === 'feedback' && opt === selectedOption && opt !== currentQ.correctAnswer && (
          ...
          <AnimatePresence>
            {quizState === 'feedback' && (
              <motion.div 
                ...
                className="mt-6 p-4 rounded-xl bg-gold-500/10 border border-gold-500/20"
              >
                <p className="text-xs font-bold text-gold-400 uppercase tracking-widest mb-2">
                  Explanation (Next in {transitionTimer}s)
                </p>
    ...
    const handleScroll = () => {
      ...
      for (const section of sections) {
        if (section.ref.current) {
          ...
          // Check if section is in view (top is less than 200px from top of viewport)
  ...
  return (
    <div className="min-h-screen font-sans text-slate-800">
            ...
            >
              <div className="p-6 md:p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                ...
                <button 
                  ...
                  className="p-3 bg-white border border-slate-200 rounded-full hover:bg-slate-100 transition shadow-sm"
                >
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="p-6 bg-slate-50 border-t border-slate-100 flex justify-end">
                <button 
                  onClick={() => setLegalModal(null)} 
                  className="px-10 py-3 bg-midnight-950 text-white rounded-xl font-bold shadow-lg hover:bg-slate-800 transition"
                >
                  Got it
                </button>
              </div>
      ...
      <header className={`fixed top-0 w-full z-50 transition-all duration-500 ${scrolled ? 'bg-white border-b border-slate-200 py-4 shadow-sm' : 'bg-transparent py-6'}`}>
        <div className="container mx-auto px-6 md:px-12 flex justify-between items-center">
          ...
          >
            ...
            <span className="text-2xl font-serif font-bold text-midnight-950 tracking-tight">
              GK <span className="text-gold-600">Guruji</span>
            </span>
          </div>
          <nav className="hidden md:flex items-center gap-10">
            {['features', 'pricing'].map((item) => (
              <button 
                ...
                className={`text-xs font-bold uppercase tracking-[0.15em] hover:text-gold-600 transition-colors ${activeSection === item ? 'text-gold-600' : 'text-slate-500'}`}
              >
                {item}
              </button>
            ))}
          ...
          <div className="hidden md:flex items-center gap-4">
            <a 
              href={TELEGRAM_LINK} 
              className="group relative bg-midnight-950 text-white px-7 py-3 rounded-full font-bold shadow-lg overflow-hidden"
            >
              ...
              <span className="relative flex items-center gap-2 text-sm z-10 group-hover:text-midnight-950 transition-colors">
                {t.nav.cta} <Send size={14} />
              </span>
            </a>
          </div>
        </div>
        ...
        <div className="bg-midnight-950/90 rounded-2xl shadow-2xl p-2 flex justify-between items-center border border-white/10">
          {bottomNavItems.map((item) => (
            <button 
              ...
              className={`flex-1 flex flex-col items-center py-2 rounded-xl transition-all ${activeSection === item.id ? 'text-gold-400 bg-white/10' : 'text-slate-400'}`}
            >
              <item.icon size={20} />
          ...
          <a 
            href={TELEGRAM_LINK} 
            className="flex-1 flex flex-col items-center py-2 text-gold-500"
          >
            <div className="w-10 h-10 bg-gold-500 rounded-full flex items-center justify-center text-midnight-950 shadow-glow-gold -mt-6 border-4 border-midnight-900">
      ...
      <section 
        id="hero" 
        ref={heroRef}
        className="relative pt-32 pb-20 md:pt-48 md:pb-40 overflow-hidden bg-midnight-950 text-white"
      >
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <motion.div initial={false} animate="visible" variants={staggerContainer}>
              <motion.div variants={fadeInUp} className="inline-flex items-center gap-2 bg-white/5 border border-white/10 px-4 py-1.5 rounded-full mb-8">
                ...
                <span className="text-xs font-bold text-gold-200 tracking-wider uppercase">
                  {t.hero.badge}
                </span>
              </motion.div>
              <motion.h1 variants={fadeInUp} className="text-5xl md:text-7xl font-serif font-bold leading-[1.1] mb-6">
                Unlock Your <br/>
                <span className="text-transparent bg-clip-text bg-gold-gradient">Dream Job.</span>
              </motion.h1>
              <motion.p variants={fadeInUp} className="text-lg text-slate-300 mb-10 max-w-lg font-light">
                {t.hero.subheadline}
              </motion.p>
              <motion.div variants={fadeInUp} className="flex flex-col gap-4">
                <button 
                  onClick={() => setIsQuizActive(true)} 
                  className="w-full sm:w-auto group relative bg-gradient-to-r from-red-600 to-red-500 text-white px-8 py-5 rounded-2xl font-bold text-lg shadow-xl overflow-hidden"
                >
                  <span className="relative flex items-center justify-center gap-3">
                    <Play size={24} fill="currentColor" /> Practice 50 GK questions
                  </span>
                </button>
                <div className="flex flex-col sm:flex-row gap-4 mt-2">
                  <a 
                    href={TELEGRAM_LINK} 
                    className="bg-gold-500 text-midnight-950 px-8 py-4 rounded-xl font-bold text-lg shadow-glow-gold flex items-center justify-center gap-3 transition-transform hover:-translate-y-1"
                  >
                    {t.hero.ctaPrimary} <Zap size={20} fill="currentColor" />
                  </a>
                  <button 
                    onClick={() => scrollToSection('pricing')} 
                    className="bg-white/5 text-white border border-white/10 px-8 py-4 rounded-xl font-bold text-lg transition-colors hover:bg-white/10"
                  >
                    View Plans
                  </button>
                </div>
            ...
            <div className="relative hidden lg:block perspective-1000 animate-float">
              <div className="relative w-full max-w-sm mx-auto aspect-[4/5] bg-white/10 border border-white/20 rounded-[2.5rem] shadow-2xl p-8 flex flex-col justify-between">
                ...
                <div className="my-auto">
                  <h3 className="text-2xl font-serif font-bold text-white mb-2">Daily Challenge</h3>
                  <p className="text-slate-300 text-sm mb-6">Compete with 50k+ students.</p>
                </div>
                <div className="bg-midnight-900/50 rounded-xl p-4 flex items-center gap-3 border border-white/5">
                  <div className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center text-xs">
                    GK
                  </div>
                  <div>
                    <p className="text-xs text-slate-400">Join the elite</p>
                    <p className="text-sm font-bold text-white">GK Guruji Premium</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="container mx-auto px-6 md:px-12">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {t.features.list.map((feature, idx) => (
              ...
              >
                <div className="w-12 h-12 bg-slate-50 rounded-2xl flex items-center justify-center mb-6 text-midnight-900 group-hover:bg-gold-500 group-hover:text-white transition-colors">
                  <IconMap name={feature.iconName} />
                </div>
      ...
      >
        <div className="container mx-auto px-6 md:px-12 text-center mb-16">
          <h2 className="text-3xl md:text-5xl font-serif font-bold text-midnight-950 mb-4">
            Premium Membership
          </h2>
        </div>
          ...
          {t.pricing.plans.map((plan, idx) => (
            <div 
              ...
              className={`relative rounded-[2rem] p-8 transition-all border overflow-hidden ${
                plan.id === 'basic' 
                  ...
                  : plan.id === 'pro' 
                  ? 'bg-gradient-to-br from-amber-800 to-amber-950 text-white shadow-2xl scale-105 border-amber-700'
            ...
            >
              ...
              <h3 className={`text-lg font-bold mb-2 ${
                plan.recommended ? 'text-amber-100' : 
                plan.id === 'basic' ? 'text-amber-700' : 'text-stone-400'
              }`}>
                {plan.name}
              </h3>
